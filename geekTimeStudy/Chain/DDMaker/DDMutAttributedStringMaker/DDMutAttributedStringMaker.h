...
@interface DDMutAttributedStringMaker : NSObject
/** 字体 */
@property(nonatomic,copy) DDMutAttributedStringMaker *(^font)(UIFont *font);
@property(nonatomic,copy) DDMutAttributedStringMaker *(^fontRange)(UIFont *font,NSRange range);

/** 文字颜色 */
@property(nonatomic,copy) DDMutAttributedStringMaker *(^textColor)(UIColor *textColor);
@property(nonatomic,copy) DDMutAttributedStringMaker *(^textColorRange)(UIColor *textColor,NSRange range);

/** 段落 */
@property(nonatomic,copy) DDMutAttributedStringMaker *(^paragraphStyle)(NSParagraphStyle *paragraphStyle);
@property(nonatomic,copy) DDMutAttributedStringMaker *(^paragraphStyleRange)(NSParagraphStyle *paragraphStyle,NSRange range);

//行间距(竖直)
@property(nonatomic,copy) DDMutAttributedStringMaker *(^lineSpacing)(CGFloat lineSpacing);
@property(nonatomic,copy) DDMutAttributedStringMaker *(^lineSpacingRange)(CGFloat lineSpacing,NSRange range);

//字间距(横向) 次数注意参数类型为NSNumber.
@property(nonatomic,copy) DDMutAttributedStringMaker *(^kern)(NSNumber *kern);
@property(nonatomic,copy) DDMutAttributedStringMaker *(^kernRange)(NSNumber *kern,NSRange range);

/**
 *【only one】某个指定的特别字符串处理
 * ☠☠☠ 注意: 就算有多个相同的字符串，也只处理第一个☠☠☠
 */
@property(nonatomic,copy) DDMutAttributedStringMaker *(^specialText)(NSString *specialText,UIFont *specialFont,UIColor *specialColor);
/**
 *【one or more】某些指定的字符串快速处理集合, ☆☆ 数组个数最好相同 ☆☆ 。
 */
@property(nonatomic,copy) DDMutAttributedStringMaker *(^specialTextSet)(NSArray<NSString *> *specialTextArray,NSArray<UIFont *> *specialTextFontArray,NSArray<UIColor *> *specialTextColorArray);
@end


@interface NSMutableAttributedString (DDMaker)
/**
 * 返回一个NSMutableAttributedString对象，同时要注意初始化时是需要一个非空str的。
 * 参考`NSAttributedString`的` - (instancetype)initWithString:(NSString *)str`方法;
 */
+(NSMutableAttributedString *)dd_attributedStringMake:(void(^)(DDMutAttributedStringMaker *maker))make str:(NSString * _Nonnull)str;
